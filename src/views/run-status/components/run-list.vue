<template>
  <a-row>
    ...
    <a-col :flex="'86px'" style="text-align: right">
      <a-space direction="vertical" :wrap="true" :size="18">
  ...
  >
    ...
    <template v-if="scheduleType === 'timer'" #scheduleStatus="{ record }">
    ...
    <template #operations="{ record }">
      <a-space direction="horizontal">
        ...
        <a-space v-if="formModel.schedule_type == 'once'">
          <a-popconfirm
        ...
        <a-space>
          <a-popconfirm
            v-if="formModel.schedule_type == 'once'"
            :content="$t('job.action.confirm.stop')"
            @before-ok="handleAction($event, record, 'Stop')"
          >
        ...
        <a-space v-if="formModel.schedule_type == 'timer'">
          <a-popconfirm
            :content="$t('job.action.confirm.startTimer')"
            @before-ok="handleAction($event, record, 'StartTimer')"
          >
            <a-button type="dashed" size="mini" status="success">
        ...
        <a-space v-if="formModel.schedule_type == 'timer'">
          <a-popconfirm
            ...
            @before-ok="handleAction($event, record, 'StopTimer')"
          >
            <a-button type="dashed" size="mini" status="danger">
...
<script lang="ts" setup>
  ...
  const generateFormModel = () => {
    return {
      schedule_name: '',
      job_type: 'default',
      schedule_type: (props.scheduleType as string) || 'once',
      bind_ip: '',
    };
  };
  ...
  const columns = computed<TableColumnData[]>(() => {
    ...
    if (formModel.value.schedule_type === 'timer') {
      const sli: TableColumnData[] = [
        {
    ...
    return vals.filter((val) => {
      if (
        ['schedule_status'].includes(val.dataIndex as string) &&
        formModel.value.schedule_type === 'once'
      ) {
        return false;
      }
      return true;
  ...
  const handleAction = async (
    e: any,
    record: any,
    action: 'Exec' | 'Stop' | 'StartTimer' | 'StopTimer'
  ) => {
    await jobAction({
      action,
