<template>
  <div style="width: 100%">
    <a-row v-if="props.controlled">
      <a-button type="secondary" @click="addArg" size="mini">
        {{ $t('form.add') }}
      </a-button>
    </a-row>

    <a-row v-if="args.length > 0">
      <a-table
        :data="args"
        :columns="columns"
        style="margin-top: 5px"
        size="small"
        :pagination="false"
      >
        <template #name="{ rowIndex }">
          <a-input
            v-model="args[rowIndex].name"
            @change="changeValue"
            :disabled="!props.controlled"
          />
        </template>
        <template #val="{ rowIndex }">
          <a-input v-model="args[rowIndex].val" @change="changeValue" />
        </template>
        <template #info="{ rowIndex }">
          <a-input
            v-model="args[rowIndex].info"
            @change="changeValue"
            :disabled="!props.controlled"
          />
        </template>
        <template v-if="props.controlled" #operations="{ rowIndex }">
          <a-button size="mini" @click="deleteArg(rowIndex)">
            {{ $t('operations.delete') }}
          </a-button>
        </template>
      </a-table>
    </a-row>
  </div>
</template>
<script lang="ts" setup>
  import { useI18n } from 'vue-i18n';
  import { WorkflowUserVariables } from '@/api/workflow';
  import { computed, PropType, ref, watch } from 'vue';

  const props = defineProps({
    args: {
      type: Array as PropType<WorkflowUserVariables[]>,
      default() {
        return [];
      },
    },
    controlled: {
      type: Boolean,
      default: false,
    },
  });

  const { t } = useI18n();
  const emit = defineEmits(['update:args']);
  const args = ref(props.args);

  const columns = computed(() => {
    const cols = [
      {
        title: t('workflow.userVariables.name'),
        dataIndex: 'name',
        slotName: 'name',
      },
      {
        title: t('workflow.userVariables.defVal'),
        dataIndex: 'val',
        slotName: 'val',
      },
      {
        title: t('workflow.userVariables.info'),
        dataIndex: 'info',
        slotName: 'info',
      },
    ];
    if (props.controlled) {
      cols.push({
        title: t('operations'),
        dataIndex: 'operations',
        slotName: 'operations',
      });
    }

    return cols;
  });

  const changeValue = () => {
    emit('update:args', args.value);
  };

  const addArg = () => {
    args.value.push({ name: '', val: '', info: '' });
    emit('update:args', args.value);
  };

  const deleteArg = (i: number) => {
    args.value.splice(i, 1);
    emit('update:args', args.value);
  };
</script>
